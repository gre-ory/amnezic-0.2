...
class Insert( Sql ):
    ...
    def __init__( self, table, **kwargs ):
        ...
        self.query = 'insert into %s ( %s ) values ( %s )' % ( self.table.name, ', '.join( self.columns ), ', '.join( [ '?' * len( self.args ) ] ) )    
...
def retrieve_all( table ):
    for row in table.select( 'json' ).order_by( 'id desc' ).fetch_all():
        print row[ 'json' ]
        print util.Object( row[ 'json' ] )
        print util.Object( row[ 'json' ] ).json
        print util.Object( row[ 'json' ] ).indented_json
    return [ util.Object( row[ 'json' ] ) for row in table.select( 'json' ).order_by( 'id desc' ).fetch_all() if row is not None ]
...
def delete( table, oid ):
    obj = retrieve( table, oid )
    table.delete().where( oid=oid ).execute()
    return obj
